...
function Dashboard() {
  ...
  const GetResumesList = () => {
    ...
    GlobalApi.GetUserResumes(user?.primaryEmailAddress?.emailAddress)
      .then((resp) => {
        setResumeList(Array.isArray(resp.data) ? resp.data : []);
      })
      .catch((error) => {
        console.error("Error fetching resumes:", error);
        setResumeList([]); // Set to empty array in case of error
  ...
  /**
   * Remove a deleted resume from the UI
   */
  const handleDeleteResume = (deletedResumeId) => {
    setResumeList((prevList) =>
      prevList.filter((resume) => resume.data.resumeId !== deletedResumeId)
    );
  };
        ...
        {loading ? (
          ...
          resumeList.map((resumes, index) => (
            <ResumeCardItem
              resume={resumes}
              key={index}
              onDelete={handleDeleteResume} // Pass delete handler
            />
          ))
        ) : (
          // Display message if no resumes are found
